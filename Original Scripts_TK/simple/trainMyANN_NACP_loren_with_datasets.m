...
% concatenated, matched to flags, and averaged to half days, and then daylight half days are used)
% Note: tutorial available with typing nnstart

%%%%%%%%%%%%%%%%%%%%%%%
%
% Note: Need to set which column ANN is trained to, and which inputs, at
% beginning of section 2. The drivers are called by column number in the
% 'annDrivers' definition.  Also put column numbers in driversHourly.colheaders
% to show names of drivers as a way to check which drivers are which columns.
% If you get an error, make sure you cleared all first.
%%%%%%%%%%%%%%%%
...
%     NEEobsMod.data=NEEobsMod(366:end,:);
%     
% end
...
%NEEobsMod=NEEobsMod.data; %With the dataset imported it becomes a structure.  This .data command accesses the data (excluding headers)

driversDaily = driversHourly.data; %Processed Ameriflux data is already half-day averaged.  The .data accesses the data part of the structure
end
...
driversHourly.colheaders(1,[6,17,29]) %%Choose subset of drivers (LPA edited)
%annDrivers = subDriversMonths(:,6)'; %Show column names of selected drivers (LPA edited)

% set what the ANN is trained to
%targets = NEEobsMod.data(:,3)';
targets = subDataMonths(:,6)'; %column 6 for processed Ameriflux data (LPA edited)
...
xlabel(strcat('Half-day since January 1, ',num2str(yearStart)))
